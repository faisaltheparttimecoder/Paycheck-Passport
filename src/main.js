...
async function init() {
  try {
    ...
    const [countries, _rates] = await Promise.all([
      loadCountries(),
      loadRates()
    ]);
...
function handleFormChange(state) {
  ...
  const results = countriesToCalculate.map(iso => {
    ...
    if (country.tier === 1 && calculators[iso]) {
      ...
      calculation = calculateRough(salaryInLocalCurrency, country.estimated_rate || 0.30);
    } else {
      // Fallback rough estimate
      calculation = calculateRough(salaryInLocalCurrency, 0.30);
    }
...
function showError(message) {
  ...
  error.style.cssText = 'background: #fee2e2; color: #991b1b; padding: 1rem; border-radius: 8px; margin: 1rem 0;';
